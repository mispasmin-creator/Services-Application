...
import { cn, formatCurrency, uploadFileToDrive, nowDateTime, getDriveViewUrl, formatDate } from '../lib/utils';
...
const Offers = () => {
  ...
  const handleCreateOffer = async (e) => {
    ...
    try {
      ...
      if (res && res.success !== false) {
        ...
        alert(`Offer ${autoId} successfully create ho gaya hai!`);
      } else {
        const errMsg = res?.message || res?.error || 'Failed to save offer.';
        setSaveError(errMsg);
        alert('Error: ' + errMsg);
      }
    } catch (err) {
  ...
  const handleConvertSubmit = async (e) => {
    ...
    try {
      ...
      if (serviceRes.success) {
        // addService already calls fetchData() which auto-refreshes OFFER status from sheet formulas
        // No need to call updateOffer — OFFER sheet should not be touched during service creation
  ...
  const filteredOffers = offers.filter(o => {
    ...
    // Firm filter
    if (firmFilter && (o.firmName || '').toLowerCase() !== firmFilter.toLowerCase()) return false;
            ...
            <form onSubmit={handleCreateOffer} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-700 uppercase">Offer No.</label>
                  <input
                    type="text"
                    disabled
