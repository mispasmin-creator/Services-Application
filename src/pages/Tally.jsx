    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Perform audits, handle rectification cases, and record tally voucher entries.</p>
        </div>
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border-l-4 border-l-amber-500 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Awaiting Audit / Rectification</p>
            <h4 className="text-2xl font-bold text-slate-900 mt-1">{auditCount + rectifyCount} <span className="text-xs font-normal text-slate-400">entries</span></h4>
          </div>
          <div className="p-3 bg-amber-50 rounded-xl">
            <ShieldAlert className="text-amber-500" size={24} />
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border-l-4 border-l-indigo-500 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Ready for Tally Entry</p>
            <h4 className="text-2xl font-bold text-slate-900 mt-1">{tallyCount} <span className="text-xs font-normal text-slate-400">entries</span></h4>
          </div>
          <div className="p-3 bg-indigo-50 rounded-xl">
            <Database className="text-indigo-500" size={24} />
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border-l-4 border-l-emerald-500 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Tallied Amount</p>
            <h4 className="text-2xl font-bold text-slate-900 mt-1">{formatCurrency(totalAmountCompleted)}</h4>
          </div>
      ...
      {loading ? (
        <div className="py-12 flex flex-col items-center justify-center gap-2 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-slate-400 text-sm font-medium">Fetching staging data...</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Paid To / Vendor</th>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Net Amount</th>
                  
                  {activeTab === 'rectify' && (
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Rectification Reason</th>
                  )}
                  {activeTab === 'completed' && (
                    <>
                      <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Voucher No</th>
                      <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Tally Date</th>
                    </>
                  )}
              ...
              <tbody className="divide-y divide-slate-200">
                {activeTabItems.map((item, index) => (
                  <tr key={`tally-stage-${item.type}-${item.sheetRowIndex}-${index}`} className="hover:bg-slate-50/70 transition-colors">
                    {/* Item Type */}
                    ...
                    {/* Paid To */}
                    <td className="px-6 py-4 text-sm font-medium text-slate-600">{item.paidTo}</td>
                    ...
                    {activeTab === 'completed' && (
                      <>
                        <td className="px-6 py-4 text-sm font-bold text-slate-700">
                          {item.tallyVoucher || <span className="text-slate-300 italic">N/A</span>}
                        </td>
                ...
                {activeTabItems.length === 0 && (
                  <tr>
                    <td colSpan={activeTab === 'rectify' ? 8 : (activeTab === 'completed' ? 9 : 7)} className="px-6 py-12 text-center text-slate-400 text-sm">
                      <div className="flex flex-col items-center gap-2">
                        <Database className="text-slate-300" size={32} />
                        <p className="font-semibold text-slate-400">No records found in this stage.</p>
                        <p className="text-xs text-slate-300">Filtered entries mapped to '{activeTab}' are currently empty.</p>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden border border-slate-100">
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">Prefilled details. Mark as Done or Not Done.</p>
              </div>
              <button 
                disabled={isSaving}
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100 text-xs">
                <div>
                  <span className="text-slate-400 font-bold block uppercase">Payee / Vendor</span>
                  <span className="text-slate-800 font-bold block mt-0.5">{selectedItem.paidTo}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-bold block uppercase">Transaction Type</span>
                  <span className="text-slate-800 font-bold block mt-0.5">{selectedItem.type}</span>
                </div>
                <div>
                ...
                <div>
                  <span className="text-slate-400 font-bold block uppercase">TDS Deducted</span>
                ...
                <div className="col-span-2 border-t border-slate-200/50 pt-2">
                  <span className="text-slate-400 font-bold block uppercase">Description / Purpose</span>
                  <span className="text-slate-700 font-medium block mt-0.5">{selectedItem.description || selectedItem.remarks || '—'}</span>
                </div>
              </div>
              ...
              {(selectedItem.billCopy || selectedItem.billImage || selectedItem.paymentProof) && (
                <div className="space-y-1.5 p-3.5 bg-blue-50/50 border border-blue-100/60 rounded-xl text-xs">
                  <span className="text-slate-500 font-bold block uppercase">Uploaded Attachments</span>
              ...
              {showRectificationInput ? (
                <div className="space-y-2 border-t border-slate-100 pt-3">
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-rose-500 focus:outline-none"
                  />
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    <button
                      ...
                      className="px-3 py-1.5 border border-slate-200 rounded-lg text-slate-500 text-xs font-semibold hover:bg-slate-50"
                    >
                      Back
                    </button>
                    <button
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5 border-t border-slate-100 pt-3">
                    <label className="text-xs font-bold text-slate-700 uppercase">Remarks / Audit Notes</label>
                    <textarea
                      disabled={isSaving}
                      rows={2}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden border border-slate-100">
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">Fix issue and mark as rectified</p>
              </div>
              <button 
                disabled={isSaving}
            ...
            <form onSubmit={handleRectifySubmit} className="p-6 space-y-4">
              ...
              {/* Prefilled Display */}
              <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-3.5 border border-slate-100 rounded-xl">
                <div>
                  <span className="text-slate-400 block font-bold uppercase">Payee:</span>
                  <span className="text-slate-800 block font-bold">{selectedItem.paidTo}</span>
                </div>
                <div>
                  <span className="text-slate-400 block font-bold uppercase">Net Amount:</span>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase">Resolution Notes / Action Taken</label>
                <textarea
                  disabled={isSaving}
                  rows={2}
            ...
            <form onSubmit={handleTallyEntry} className="p-6 space-y-4">
              ...
              <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 space-y-2">
                <div className="grid grid-cols-2 gap-y-1.5 text-xs">
                  ...
                  <span className="font-bold text-slate-800 text-right">{selectedItem.paidTo}</span>
                  ...
                  <span className="text-slate-700 font-bold border-t border-indigo-200 pt-1">Net Amount:</span>
              ...
              {selectedItem.type === 'Service' && (selectedItem.remarks3 || selectedItem.remarks4) && (
                <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-xs space-y-2">
                  <span className="font-bold text-slate-500 uppercase tracking-wider block">Remarks History</span>
                  {selectedItem.remarks3 && (
                    <div className="border-b border-slate-200/40 pb-1">
                      <span className="font-semibold text-slate-400 block">Audit Stage Remarks:</span>
                      <p className="text-slate-700 italic font-medium mt-0.5">"{selectedItem.remarks3}"</p>
                    </div>
                  )}
                  {selectedItem.remarks4 && (
                    <div>
                      <span className="font-semibold text-slate-400 block">Rectify Stage Remarks:</span>
                      <p className="text-slate-700 italic font-medium mt-0.5">"{selectedItem.remarks4}"</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {selectedItem.type === 'Utility' && selectedItem.remarks && (
                <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-xs space-y-1">
                  ...
                  <div>
                    ...
                    <p className="text-slate-700 italic font-medium mt-0.5">"{selectedItem.remarks}"</p>
                  </div>
                </div>
              )}

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase">Tally Entry Date *</label>
                <input
                  type="date"
                  value={tallyDate}
                  onChange={(e) => setTallyDate(e.target.value)}
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none font-semibold"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase">Tally Voucher No. / Reference</label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase">Remarks / Accounting Notes</label>
                <textarea
                  disabled={isSaving}
                  rows={2}
